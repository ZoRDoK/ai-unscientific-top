// Refactored by builder-3 (qwen3.6-plus)
/**
...
export class CallbackDispatchService {
  ...
  static async create(
    ...
    const promptPaths = [
      REWRITE_CALLBACK_PROMPT_PATH,
      REWRITE_PUBLISH_PROMPT_PATH,
      REWRITE_PUBLISH_WITH_FEEDBACK_PROMPT_PATH,
    ];

    const loadPromises = promptPaths.map((path) => readFile(path, 'utf-8'));
    const promptContents = await Promise.all(loadPromises);
    const trimmedPrompts = promptContents.map((content) => content.trim());
      ...
      logger.debug(
        {
          ...
          hasData: Boolean(callbackData),
          hasMessage: Boolean(callbackMessage),
        },
  ...
  private async handleApproveOrReject(context: CallbackContext): Promise<void> {
    const { postId, callbackId, chatId, messageId, action } = context;
  ...
  private async handleRewriteAction(context: CallbackContext): Promise<void> {
    const { postId, callbackId, chatId } = context;
    if (!postId) return;
    await this.handleRewrite(postId, callbackId, chatId);
  }
  ...
  private async handleRewriteNow(context: CallbackContext): Promise<void> {
    ...
    this.clearPending(chatId, 'rewrite_feedback');

    await this.telegramService.answerCallbackQuery(callbackId, {
      text: '🔄 Rewriting...',
    });

    await this.doRewrite(postId, chatId, undefined);
  }

  private async handleSelect(context: CallbackContext): Promise<void> {
    const { postId, imageId, callbackId, chatId, messageId } = context;
    if (!postId || imageId === undefined) return;
    await this.handleSelectImage(postId, imageId, callbackId, chatId, messageId);
  }

  private async handlePublishedAction(context: CallbackContext): Promise<void> {
    const { postId, callbackId, chatId, messageId } = context;
    if (!postId) return;
    await this.handlePublished(postId, callbackId, chatId, messageId);
  }

  private async handleRewritePublishAction(context: CallbackContext): Promise<void> {
    const { postId, callbackId, chatId, messageId } = context;
    if (!postId) return;
    await this.handleRewritePublish(postId, callbackId, chatId, messageId);
  }
  ...
  private async handleRewritePublishNow(context: CallbackContext): Promise<void> {
    ...
    this.clearPending(chatId, 'rewrite_publish_feedback');

    await this.telegramService.answerCallbackQuery(callbackId, {
      text: '🔄 Rewriting...',
    });

    await this.doRewritePublish(postId, chatId, undefined);
  }

  private async handleUploadImageAction(context: CallbackContext): Promise<void> {
    const { postId, callbackId, chatId } = context;
    if (!postId) return;
    await this.handleUploadImage(postId, callbackId, chatId);
  }

  private async handleSearchImagesAction(context: CallbackContext): Promise<void> {
    const { postId, callbackId, chatId } = context;
    if (!postId) return;
    await this.handleSearchImages(postId, callbackId, chatId);
  }

  private async handlePublishWithoutImageAction(context: CallbackContext): Promise<void> {
    const { postId, callbackId, chatId, messageId } = context;
    if (!postId) return;
    await this.handlePublishWithoutImage(postId, callbackId, chatId, messageId);
  }

  private async handleCancelSearch(context: CallbackContext): Promise<void> {
    const { callbackId, chatId } = context;
    this.clearPending(chatId, 'image_search');
    await this.telegramService.answerCallbackQuery(callbackId, {
      text: 'Cancelled',
    });
  }
  ...
  private async handleBackToRewriteAction(context: CallbackContext): Promise<void> {
    const { postId, callbackId, chatId } = context;
    if (!postId) return;
    await this.handleBackToRewrite(postId, callbackId, chatId);
  }
  ...
  private async handleResetFromRewrite(context: CallbackContext): Promise<void> {
    ...
    await this.doRewrite(postId, chatId, undefined, false);
  }

  private async handleSelectImagesAction(context: CallbackContext): Promise<void> {
    const { postId, callbackId, chatId } = context;
    if (!postId) return;
    await this.handleSelectImages(postId, callbackId, chatId);
  }

  private async handleResetAction(context: CallbackContext): Promise<void> {
    const { postId, callbackId, chatId, messageId } = context;
    if (!postId) return;
    await this.handleResetToReview(postId, callbackId, chatId, messageId);
  }
  ...
  private async handleRewrite(postId: number, callbackId: string, chatId: string): Promise<void> {
    ...
    const message = `✏️ Write what's wrong with post #${postId} text (or press "No comment"):`;

    await this.telegramService.sendMessage(chatId, message, { reply_markup: keyboard });
  }
  ...
  private async doRewrite(
    ...
    try {
      ...
      if (!rewriteResult.success || !rewriteResult.summary) {
        const errorText = rewriteResult.error || 'MiniMax returned empty response';
        const message = `❌ ${errorText} for post #${postId}. Try "Rewrite" again.`;
  ...
  private async finalizeStatusMessage(
    ...
    await this.answerCallbackQuerySafe(callbackId, statusLabel);
    await this.clearMessageMarkupSafe(chatId, messageId);
    await this.editMessageTextSafe(chatId, messageId, statusLabel, summary);
  }

  private async answerCallbackQuerySafe(callbackId: string, text: string): Promise<void> {
    try {
      await this.telegramService.answerCallbackQuery(callbackId, { text });
    } catch (e: unknown) {
  ...
  private async clearMessageMarkupSafe(chatId: string, messageId: number): Promise<void> {
    try {
  ...
  private async editMessageTextSafe(
    chatId: string,
    messageId: number,
    statusLabel: string,
    summary?: string
  ...
  private async finalizeReviewMessage(
    ...
    try {
      ...
      const errorMsg = e instanceof Error ? e.message : String(e);
      logger.error(
        { chatId, messageId, error: errorMsg },
  ...
  private async sendNextForReview(chatId: string): Promise<void> {
    try {
      ...
      const errorMsg = e instanceof Error ? e.message : String(e);
      logger.error({ chatId, error: errorMsg }, 'failed to send next review post');
    }
  }
  ...
  private async sendPostForPublish(post: PostWithDetails, chatId: string): Promise<void> {
    ...
    if (!result.success) {
      ...
      await this.telegramService.sendMessage(
        chatId,
        `❌ Failed to send post #${post.id} for publishing: ${errorMessage}`
      );
    }
  }
  ...
  private async handleRewritePublish(
    ...
    const message = `✏️ Write what to fix in post #${postId} (or press "No comment"):`;
  ...
  private async doRewritePublish(
    ...
    try {
      ...
      const message = `📋 Post #${postId} rewritten. Press "Published" when you forward it to the channel.`;
  ...
  private async handleSearchImages(
    ...
    const message = `🔍 Enter a search query for post #${postId}:`;
  ...
  private async doSearchImages(postId: number, chatId: string, query: string): Promise<void> {
    ...
    try {
      ...
      if (!result.success) {
        ...
        await this.handleSearchError(chatId, postId, normalizedQuery, error);
        return;
      }
  ...
  private async sendImageMediaGroup(
    ...
    const downloadable = await this.filterDownloadableImages(imageState.foundImages);

    if (downloadable.length === 0) {
      return;
    }

    const mediaItems = this.buildMediaItems(downloadable, postId, imageState);
  ...
  private async filterDownloadableImages(
    images: PostImage[]
  ): Promise<(PostImage & { local_path: string })[]> {
    ...
    return results.filter((img): img is PostImage & { local_path: string } => img !== null);
  }

  private buildMediaItems(
    downloadable: (PostImage & { local_path: string })[],
    postId: number,
    imageState: {
      originalLocalPath?: string;
      hasOriginalLocal: boolean;
    }
  ): Array<{ type: 'photo'; media: string; caption?: string }> {
