...
type ActionHandler = (context: CallbackContext) => Promise<void>;
...
export class CallbackDispatchService {
  ...
  private async handleMessage(message: TelegramMessage): Promise<void> {
    ...
    // Clear pending state first (most operations need fresh state)
  ...
  private async handleCallbackQuery(callbackQuery: TelegramCallbackQuery): Promise<void> {
    ...
    const parsed = parseCallbackData(callbackQuery.data as string);
    const message = callbackQuery.message as { chat: { id: number }; message_id: number };
  ...
  private async routeCallback(context: CallbackContext): Promise<void> {
    ...
    await this.safeCallbackAnswer(this.telegramService, context.callbackId, 'Unknown action');
  }

  // ── Action handlers (thin wrappers) ────────────────────────────────

  private async handleApproveOrReject(context: CallbackContext): Promise<void> {
    if (!context.postId) {
      await this.safeCallbackAnswer(this.telegramService, context.callbackId, '❌ Error: postId not specified');
      return;
    }
  ...
  private async handleRewriteNow(context: CallbackContext): Promise<void> {
    ...
    await this.safeCallbackAnswer(this.telegramService, context.callbackId, '🔄 Rewriting...');
  ...
  private async handleCancelSearch(context: CallbackContext): Promise<void> {
    ...
    await this.safeCallbackAnswer(this.telegramService, context.callbackId, 'Cancelled');
  }
  ...
  private async handleResetFromRewrite(context: CallbackContext): Promise<void> {
    if (!context.postId) return;
    await this.safeCallbackAnswer(this.telegramService, context.callbackId, '🔄 Regenerating...');
  ...
  private async handleRewrite(postId: number, callbackId: string, chatId: string): Promise<void> {
    await this.safeCallbackAnswer(this.telegramService, callbackId, '✏️ Write what\'s wrong');
  ...
  private async doRewrite(
    ...
    const post = await this.fetchPostOrError(chatId, postId, 'fetch post for rewrite');
    if (!post) return;
    ...
    await this.generateSummaryOrError(chatId, postId, summary);
  ...
  private async fetchPostOrError(
    chatId: string,
    postId: number,
    operationName: string
  ): Promise<PostWithDetails | null> {
    const post = await this.withErrorLog(
      () => this.backend.getPost(postId),
      { postId, chatId },
      operationName
    );
    if (!post) {
      await this.telegramService.sendMessage(chatId, `❌ Failed to load post #${postId}`);
    }
    return post ?? null;
  }

  private async generateSummaryOrError(
    chatId: string,
    postId: number,
    summary: string
  ): Promise<void> {
    await this.withErrorLog(
      () => this.backend.generateSummary(postId, summary),
      { postId, chatId },
      'generateSummary'
    );
  }
  ...
  private async handleReviewAction(
    ...
    const result = await this.withErrorLog(
      () => this.backend.reviewPost(postId, action),
      { postId, action, chatId, messageId },
      'reviewPost'
    );

    if (result === undefined) {
      await this.safeCallbackAnswer(this.telegramService, callbackId, 'Error');
      return;
    }
  ...
  private async sendNextForReview(chatId: string): Promise<void> {
    const result = await this.withErrorLog(
      () => this.backend.getNextForReview({ ignoreCooldown: true }),
      { chatId },
      'getNextForReview'
    );
    if (!result) return;
    ...
    await this.withErrorLog(
      () => this.sendReviewPost(chatId, result.post),
      { chatId },
      'sendReviewPost'
    );
  }
  ...
  private async sendReviewPost(
    ...
    await this.withErrorLog(
      () => this.backend.updatePost(post.id, { user_feedback: 'review_sending' }),
      { postId: post.id },
      'updatePost review_sending'
    );
    ...
    try {
      ...
      await this.withErrorLog(
        () => this.backend.updatePost(post.id, { user_feedback: null }),
        { postId: post.id },
        'updatePost review_failed'
      );
      throw tgErr;
    }

    await this.withErrorLog(
      () => this.backend.updatePost(post.id, { user_feedback: 'review_sent' }),
      { postId: post.id },
      'updatePost review_sent'
    );
  }
  ...
  private async handleSelectImage(
    ...
    await this.withErrorLog(
      () => this.backend.selectImage(postId, imageId),
      { postId, imageId, chatId, messageId },
      'selectImage'
    );

    await this.safeCallbackAnswer(this.telegramService, callbackId, '🖼️ Image selected');
    ...
    await this.withErrorLog(
      ...
      { chatId, messageId },
      'editMessageReplyMarkup in handleSelectImage'
    );
    ...
    await this.withErrorLog(
      ...
      { chatId, messageId },
      'editMessageText in handleSelectImage'
    );

    const freshPost = await this.withErrorLog(
      () => this.backend.getPost(postId),
      { postId, chatId },
      'fetch post after image select'
    );
  ...
  private async handlePublished(
    ...
    await this.withErrorLog(
      () => this.backend.markPublished(postId),
      { postId, chatId, messageId },
      'markPublished'
    );

    const updatedPost = await this.withErrorLog(
      () => this.backend.getPost(postId),
      { postId, chatId, messageId },
      'fetch post after markPublished'
    );
  ...
  private async handleUploadImage(
    ...
    await this.safeCallbackAnswer(this.telegramService, callbackId, '📷 Send an image');
    await this.telegramService.sendMessage(
      chatId,
  ...
  private async handleRewritePublish(
    ...
    await this.safeCallbackAnswer(this.telegramService, callbackId, '✏️ Write what to fix');
  ...
  private async doRewritePublish(
    ...
    const post = await this.fetchPostOrError(chatId, postId, 'fetch post for rewrite publish');
    if (!post) return;
    ...
    const freshPost = await this.withErrorLog(
      () => this.backend.getPost(postId),
      { postId, chatId },
      'fetch fresh post in rewrite publish'
    );
    ...
    await this.telegramService.sendMessage(
      chatId,
      `📋 Post #${postId} rewritten. Press "Published" when you forward to the channel.`,
      { reply_markup: keyboard }
    );
  }
  ...
  private async handleSearchImages(
    ...
    await this.safeCallbackAnswer(this.telegramService, callbackId, '🔍 Enter a search query');
  ...
  private async doSearchImages(postId: number, chatId: string, query: string): Promise<void> {
    ...
    try {
      ...
      if (!result.success) {
        await this.handleImageSearchError(chatId, postId, normalizedQuery, result.error);
        return;
      }
  ...
  private async handleImageSearchError(
    chatId: string,
    postId: number,
    query: string,
    error?: string
    ...
    if (error === 'no_pages_found') {
      if (!this.imageSearchOrchestrator) return;
    ...
    if (error === 'no_images_found') {
      await this.telegramService.sendMessage(chatId, `😕 No images found for query: ${query}`);
      return;
    }
    ...
    logger.error({ postId, chatId, query, error }, 'image search failed');
  ...
  private async handlePublishWithoutImage(
    ...
    const result = await this.withErrorLog(
      () => this.backend.updatePost(postId, {
        status: 'ready_publish',
        selected_image_id: null,
      }),
      { postId, chatId, messageId },
      'updatePost for publish without image'
    );
    ...
    await this.safeCallbackAnswer(this.telegramService, callbackId, '📸 Sending...');
    ...
    await this.withErrorLog(
      ...
      { chatId, messageId },
      'editMessageReplyMarkup in publish without image'
    );

    await this.withErrorLog(
      () => this.telegramService.editMessageText(chatId, messageId, '<b>📸 Without image</b>', {
        parse_mode: 'HTML',
      }),
      { chatId, messageId },
      'editMessageText in publish without image'
    );

    const freshPost = await this.withErrorLog(
      () => this.backend.getPost(postId),
      { postId, chatId },
      'fetch fresh post after publish without image'
    );
  ...
  private async handleBackToRewrite(
    ...
    await this.safeCallbackAnswer(this.telegramService, callbackId, '◀️ Going back');
    const post = await this.fetchPostOrError(chatId, postId, 'fetch post for back to rewrite');
    if (!post) {
  ...
  private async handleSelectImages(
    ...
    await this.safeCallbackAnswer(this.telegramService, callbackId, '🖼️ Loading options...');
    const imageState = await this.getImageState(postId);
    const { foundImages } = imageState;
  ...
  private async getImageState(postId: number): Promise<{
    ...
    const post = await this.withErrorLog(
      () => this.backend.getPost(postId),
      { postId },
      'fetch post for image state'
    );
    if (!post) {
      return { foundImages: [], hasOriginalLocal: false };
    }
    const images = post.images || [];
  ...
  private async handleResetToReview(
    ...
    const post = await this.withErrorLog(
      () => this.backend.resetPost(postId).then(() => this.backend.getPost(postId)),
      { postId, chatId, messageId },
      'resetPost and fetch post'
    );
  ...
  private buildActionHandlers(): Map<string, ActionHandler> {
  ...
  private async safeCallbackAnswer(
    ...
    try {
      ...
      logger.debug(
        { callbackId, errMsg: e instanceof Error ? e.message : String(e) },
        'answerCallbackQuery failed (ignored)'
      );
    }
  }
  ...
  private async finalizeStatusMessage(
    ...
    await this.safeCallbackAnswer(this.telegramService, callbackId, statusLabel);
  ...
  private async withErrorLog<T>(
    ...
    try {
      ...
      logger.error({ ...errorCtx, errMsg: e instanceof Error ? e.message : String(e) }, `${operationName} failed`);
      return undefined;
    }
  }
}