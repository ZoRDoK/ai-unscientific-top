...
export class CallbackDispatchService {
  ...
  private async handleCallbackError(
    e: unknown,
    context: { callbackId: string; chatId: string; messageId?: number },
    logMessage: string,
    logExtra: Record<string, unknown> = {}
  ): Promise<void> {
    const errMsg = this.extractErrorMessage(e);
    logger.error({ ...context, ...logExtra, error: errMsg }, logMessage);
  ...
  private async doRewrite(
    ...
    try {
      ...
      logger.error({ postId, chatId, error: this.extractErrorMessage(e) }, 'rewrite callback failed');
      await this.telegramService.sendMessage(
        chatId,
  ...
  private async finalizeStatusMessage(
    ...
    try {
      ...
      logger.error(
        { callbackId, errMsg: this.extractErrorMessage(e) },
    ...
    try {
      ...
      logger.error(
        { chatId, messageId, errMsg: this.extractErrorMessage(e) },
  ...
  private async handleReviewAction(
    ...
    try {
      ...
      logger.error(
        { postId, action, chatId, messageId, error: this.extractErrorMessage(e) },
  ...
  private async fetchApprovedPost(postId: number, chatId: string): Promise<PostWithDetails | null> {
    try {
      ...
      logger.error(
        { postId, chatId, error: this.extractErrorMessage(e) },
  ...
  private async finalizeReviewMessage(
    ...
    try {
      ...
      logger.error(
        { chatId, messageId, error: this.extractErrorMessage(e) },
  ...
  private async sendNextForReview(chatId: string): Promise<void> {
    try {
      ...
      logger.error(
        { chatId, error: this.extractErrorMessage(e) },
  ...
  private async handleSelectImage(
    ...
    try {
      ...
      logger.error({ postId, imageId, chatId, messageId, error: this.extractErrorMessage(e) }, 'image selection failed');
      await this.telegramService.answerCallbackQuery(callbackId, {
        text: 'Error',
      });
    }
  ...
  private async handlePublished(
    ...
    try {
      ...
      logger.error({ postId, chatId, messageId, error: this.extractErrorMessage(e) }, 'mark published failed');
      await this.telegramService.answerCallbackQuery(callbackId, {
        text: 'Error',
      });
    }
  ...
  private async doRewritePublish(
    ...
    try {
      ...
      logger.error({ postId, chatId, error: this.extractErrorMessage(e) }, 'publish rewrite failed');
      await this.telegramService.sendMessage(
        chatId,
        `❌ Error rewriting post #${postId}`
      );
  ...
  private async handlePublishWithoutImage(
    ...
    try {
      ...
      logger.error({ postId, chatId, messageId, error: this.extractErrorMessage(e) }, 'publish without image failed');
      await this.telegramService.answerCallbackQuery(callbackId, {
        text: 'Error',
      });
    }
  ...
  private async sendImageMediaGroup(
    ...
    try {
      ...
      logger.debug({ postId, chatId, error: this.extractErrorMessage(e) }, 'failed to send media group');
    }
  }
  ...
  private async handleResetToReview(
    ...
    try {
      ...
      logger.error({ postId, chatId, messageId, error: this.extractErrorMessage(e) }, 'reset to review failed');
      await this.telegramService.answerCallbackQuery(callbackId, {
        text: 'Error',
      });
    }
