// Refactored by builder-2 (kimi-k2.5)
/**
...
export class CallbackDispatchService {
  ...
  private async handleMessage(message: TelegramMessage): Promise<void> {
    ...
    // Route text messages based on pending operation
    await this.handlePendingTextMessage(chatId, pending, text);
  }

  private async handlePendingTextMessage(
    chatId: string,
    pending: PendingState,
    text: string
  ): Promise<void> {
    const { operation, postId } = pending;

    switch (operation) {
      case 'rewrite_feedback':
        await this.doRewrite(postId, chatId, text);
        break;
      case 'rewrite_publish_feedback':
        await this.doRewritePublish(postId, chatId, text);
        break;
      case 'image_search':
        await this.doSearchImages(postId, chatId, text);
        break;
    }
  }
  ...
  private async handleApproveOrReject(context: CallbackContext): Promise<void> {
    ...
    const action = context.action as ReviewAction;
    const { postId, callbackId, chatId, messageId } = context;

    await this.handleReviewAction(postId, action, callbackId, chatId, messageId);
  }
  ...
  private async handleRewrite(postId: number, callbackId: string, chatId: string): Promise<void> {
    ...
    const messageText = `✏️ Write what's wrong with post #${postId} text (or press "No comment"):`;

    await this.telegramService.sendMessage(chatId, messageText, { reply_markup: keyboard });
  }
  ...
  private async doRewrite(
    ...
    try {
      ...
      if (!rewriteResult.success || !rewriteResult.summary) {
        const errorText = `❌ ${rewriteResult.error || 'MiniMax returned empty response'} for post #${postId}. Try "Rewrite" again.`;
        await this.telegramService.sendMessage(chatId, errorText);
        return;
      }
  ...
  private async handleReviewAction(
    ...
    try {
      ...
      const errMsg = extractErrorMessage(_e);
  ...
  private async fetchApprovedPost(postId: number, chatId: string): Promise<PostWithDetails | null> {
    try {
      ...
      logger.error(
        { postId, chatId, error: extractErrorMessage(e) },
  ...
  private async finalizeReviewMessage(
    ...
    try {
      ...
      logger.error(
        { chatId, messageId, error: extractErrorMessage(e) },
  ...
  private async finalizeStatusMessage(
    ...
    try {
      ...
      logger.error(
        { callbackId, errMsg: extractErrorMessage(e) },
        'answerCallbackQuery failed in finalizeStatusMessage'
      );
    }
    ...
    try {
      ...
      logger.error(
        { chatId, messageId, errMsg: extractErrorMessage(e) },
  ...
  private async sendNextForReview(chatId: string): Promise<void> {
    try {
      ...
      logger.error({ chatId, error: extractErrorMessage(e) }, 'failed to send next review post');
    }
  }
  ...
  private async sendPostForPublish(post: PostWithDetails, chatId: string): Promise<void> {
    ...
    if (!result.success) {
      ...
      const errorText = `❌ Failed to send post #${post.id} for publishing: ${result.error || 'unknown error'}`;
      await this.telegramService.sendMessage(chatId, errorText);
    }
  }
  ...
  private async handleRewritePublish(
    ...
    const messageText = `✏️ Write what to fix in post #${postId} (or press "No comment"):`;
    await this.telegramService.sendMessage(chatId, messageText, { reply_markup: keyboard });
  }
  ...
  private async doRewritePublish(
    ...
    try {
      ...
      const confirmText = `📋 Post #${postId} rewritten. Press "Published" when you forward it to the channel.`;
      await this.telegramService.sendMessage(chatId, confirmText, { reply_markup: keyboard });
    } catch (_e: unknown) {
