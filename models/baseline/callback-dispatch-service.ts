...
export class CallbackDispatchService {
  ...
  private get actionHandlers(): Map<string, (context: CallbackContext) => Promise<void>> {
    return this._actionHandlers;
  }

  private readonly _actionHandlers: Map<string, (context: CallbackContext) => Promise<void>> =
    new Map([
  ...
  private async doRewrite(
    ...
    try {
      ...
      await this.telegramService.sendMessage(chatId, `❌ Error rewriting post #${postId}`);
    }
  }
  ...
  private async handlePhotoMessage(
    ...
    try {
      ...
      await this.telegramService.sendMessage(chatId, `❌ Error saving image: ${errorMessage}`);
    }
  }
  ...
  private async handleSearchImages(
    ...
    await this.telegramService.sendMessage(chatId, `🔍 Enter a search query for post #${postId}:`, {
      reply_markup: keyboard,
    });
  }
