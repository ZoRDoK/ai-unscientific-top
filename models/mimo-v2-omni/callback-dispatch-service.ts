// Refactored by builder-4 (mimo-v2-omni)
/**
...
class CallbackUtils {
  ...
  static async safeAnswerCallback(
    ...
    try {
      ...
      logger.error(
        { callbackId, error: CallbackUtils.formatError(e) },
        'answerCallbackQuery failed'
      );
    }
  }

  static buildPostText(
    channelName: string,
    summary: string,
    channelTelegramId: string,
    telegramMessageId?: number
  ): string {
    const channelSlug = channelTelegramId.replace('@', '');
    const origUrl =
      channelSlug && telegramMessageId
        ? `\n\n<a href="${buildTelegramPostUrl(channelSlug, telegramMessageId)}">Source</a>`
        : '';
    return `<b>${escapeHtml(channelName)}</b>\n\n${escapeHtml(summary)}${origUrl}`;
  }
}
...
class CallbackRouter {
  private readonly actionHandlers: Map<string, (context: CallbackContext) => Promise<void>>;
  private readonly handler: CallbackHandler;

  constructor(handler: CallbackHandler) {
    this.handler = handler;
    this.actionHandlers = new Map([
      [CALLBACK_ACTION.APPROVE, this.handler.handleApproveOrReject.bind(this.handler)],
      [CALLBACK_ACTION.REJECT, this.handler.handleApproveOrReject.bind(this.handler)],
      [CALLBACK_ACTION.REWRITE, this.handler.handleRewriteAction.bind(this.handler)],
      [CALLBACK_ACTION.REWRITE_NOW, this.handler.handleRewriteNow.bind(this.handler)],
      [CALLBACK_ACTION.SELECT, this.handler.handleSelect.bind(this.handler)],
      [CALLBACK_ACTION.PUBLISHED, this.handler.handlePublishedAction.bind(this.handler)],
      [CALLBACK_ACTION.REWRITE_PUBLISH, this.handler.handleRewritePublishAction.bind(this.handler)],
      [
        CALLBACK_ACTION.REWRITE_PUBLISH_NOW,
        this.handler.handleRewritePublishNow.bind(this.handler),
      ],
      [CALLBACK_ACTION.UPLOAD_IMAGE, this.handler.handleUploadImageAction.bind(this.handler)],
      [CALLBACK_ACTION.SEARCH_IMAGES, this.handler.handleSearchImagesAction.bind(this.handler)],
      [
        CALLBACK_ACTION.PUBLISH_WITHOUT_IMAGE,
        this.handler.handlePublishWithoutImageAction.bind(this.handler),
      ],
      [CALLBACK_ACTION.CANCEL_SEARCH, this.handler.handleCancelSearch.bind(this.handler)],
      [CALLBACK_ACTION.BACK_TO_REWRITE, this.handler.handleBackToRewriteAction.bind(this.handler)],
      [CALLBACK_ACTION.RESET_FROM_REWRITE, this.handler.handleResetFromRewrite.bind(this.handler)],
      [CALLBACK_ACTION.SELECT_IMAGES, this.handler.handleSelectImagesAction.bind(this.handler)],
      [CALLBACK_ACTION.RESET, this.handler.handleResetAction.bind(this.handler)],
    ]);
  }

  async route(context: CallbackContext, telegramService: TelegramMessageService): Promise<void> {
    const handler = this.actionHandlers.get(context.action);
    if (handler) {
      await handler(context);
      return;
    }
    await telegramService.answerCallbackQuery(context.callbackId, {
      text: 'Unknown action',
    });
  }
}
...
class MessageHandler {
  ...
  constructor(pendingStateService: PendingStateService, callbackHandler: CallbackHandler) {
    this.pendingStateService = pendingStateService;
  ...
  async handleMessage(
    ...
    if (message.photo && pending.operation === 'upload_image') {
      ...
      await this.callbackHandler.handlePhotoMessage(chatId, message.photo, pending.postId);
      return;
    }
    ...
    if (pending.operation === 'rewrite_feedback') {
      await this.callbackHandler.doRewrite(pending.postId, chatId, text);
      return;
    }

    if (pending.operation === 'rewrite_publish_feedback') {
      await this.callbackHandler.doRewritePublish(pending.postId, chatId, text);
      return;
    }

    if (pending.operation === 'image_search') {
      await this.callbackHandler.doSearchImages(pending.postId, chatId, text);
      return;
    }
  }
}
...
class CallbackHandler {
  ...
  constructor(
    ...
    pendingStateService: PendingStateService,
    rewriteService: RewriteService,
    imageSearchOrchestrator: ImageSearchOrchestrator | null,
    telegramService: TelegramMessageService
  ) {
    ...
    this.imageSearchOrchestrator = imageSearchOrchestrator;
    this.telegramService = telegramService;
  }

  async handleApproveOrReject(context: CallbackContext): Promise<void> {
    if (!context.postId) {
  ...
  async handleRewriteAction(context: CallbackContext): Promise<void> {
    if (!context.postId) return;
    await this.handleRewrite(context.postId, context.callbackId, context.chatId);
  }

  async handleRewriteNow(context: CallbackContext): Promise<void> {
    if (!context.postId) return;
  ...
  async handleSelect(context: CallbackContext): Promise<void> {
  ...
  async handlePublishedAction(context: CallbackContext): Promise<void> {
    if (!context.postId) return;
  ...
  async handleRewritePublishAction(context: CallbackContext): Promise<void> {
    if (!context.postId) return;
  ...
  async handleRewritePublishNow(context: CallbackContext): Promise<void> {
    if (!context.postId) return;
  ...
  async handleUploadImageAction(context: CallbackContext): Promise<void> {
    if (!context.postId) return;
    await this.handleUploadImage(context.postId, context.callbackId, context.chatId);
  }

  async handleSearchImagesAction(context: CallbackContext): Promise<void> {
    if (!context.postId) return;
    await this.handleSearchImages(context.postId, context.callbackId, context.chatId);
  }

  async handlePublishWithoutImageAction(context: CallbackContext): Promise<void> {
    if (!context.postId) return;
  ...
  async handleCancelSearch(context: CallbackContext): Promise<void> {
  ...
  async handleBackToRewriteAction(context: CallbackContext): Promise<void> {
    if (!context.postId) return;
    await this.handleBackToRewrite(context.postId, context.callbackId, context.chatId);
  }

  async handleResetFromRewrite(context: CallbackContext): Promise<void> {
    if (!context.postId) return;
    await this.telegramService.answerCallbackQuery(context.callbackId, {
      text: '🔄 Regenerating...',
    });
  ...
  async handleSelectImagesAction(context: CallbackContext): Promise<void> {
    if (!context.postId) return;
    await this.handleSelectImages(context.postId, context.callbackId, context.chatId);
  }

  async handleResetAction(context: CallbackContext): Promise<void> {
    if (!context.postId) return;
  ...
  async handleRewrite(postId: number, callbackId: string, chatId: string): Promise<void> {
  ...
  async finalizeStatusMessage(
    ...
    try {
      ...
      const errMsg = CallbackUtils.formatError(e);
  ...
  async handleReviewAction(
    ...
    try {
      ...
      const errMsg = CallbackUtils.formatError(_e);
  ...
  async fetchApprovedPost(postId: number, chatId: string): Promise<PostWithDetails | null> {
    try {
      return await this.backend.getPost(postId);
    } catch (e: unknown) {
      logger.error(
        { postId, chatId, error: CallbackUtils.formatError(e) },
  ...
  async finalizeReviewMessage(
    ...
    try {
      ...
      logger.error(
        { chatId, messageId, error: CallbackUtils.formatError(e) },
  ...
  async sendNextForReview(chatId: string): Promise<void> {
    try {
      ...
      logger.error(
        { chatId, error: CallbackUtils.formatError(e) },
        'failed to send next review post'
      );
    }
  }

  async sendReviewPost(chatId: string, post: PostWithDetails | PostForReview): Promise<void> {
  ...
  buildReviewPostText(post: PostWithDetails | PostForReview): string {
  ...
  async buildReviewReplyMarkup(post: PostWithDetails | PostForReview) {
  ...
  async sendPostForPublish(post: PostWithDetails, chatId: string): Promise<void> {
  ...
  async handleUploadImage(postId: number, callbackId: string, chatId: string): Promise<void> {
  ...
  async handlePhotoMessage(
    ...
    try {
      ...
      const errorMessage = CallbackUtils.formatError(e);
  ...
  async handleSearchImages(postId: number, callbackId: string, chatId: string): Promise<void> {
  ...
  async doSearchImages(postId: number, chatId: string, query: string): Promise<void> {
    ...
    try {
      ...
      if (!result.success) {
        if (result.error === 'no_pages_found') {
        ...
        } else if (result.error === 'no_images_found') {
          await this.telegramService.sendMessage(
            chatId,
            `😕 No images found for query: ${normalizedQuery}`
          );
        } else if (result.error === 'no_valid_images') {
          await this.telegramService.sendMessage(
            chatId,
          ...
          await this.telegramService.sendMessage(chatId, `❌ Search error: ${result.error}`);
        }
        return;
      }
  ...
  async handlePublishWithoutImage(
    postId: number,
    callbackId: string,
    chatId: string,
    messageId: number
  ...
  async handleBackToRewrite(postId: number, callbackId: string, chatId: string): Promise<void> {
    await this.telegramService.answerCallbackQuery(callbackId, {
      text: '◀️ Going back',
    });
    const post = await this.backend.getPost(postId);
  ...
  async handleSelectImages(postId: number, callbackId: string, chatId: string): Promise<void> {
    await this.telegramService.answerCallbackQuery(callbackId, {
  ...
  async getImageState(postId: number): Promise<{
...
export class CallbackDispatchService {
  ...
  private constructor(
    ...
    const pendingStateService = createPendingStateService();
    const rewriteService = createRewriteService({ llm });
    const imageSearchOrchestrator = createImageSearchOrchestrator({ backend });
    const telegramService = new TelegramMessageService(telegram);

    if (!imageSearchOrchestrator) {
    ...
    this.callbackRouter = new CallbackRouter(this.callbackHandler);
    this.messageHandler = new MessageHandler(pendingStateService, this.callbackHandler);
  }
  ...
  private async handleCallbackQuery(callbackQuery: TelegramCallbackQuery): Promise<void> {
    ...
    await this.callbackRouter.route(context, this.telegramService);
  }

  private async handleMessage(message: TelegramMessage): Promise<void> {
    await this.messageHandler.handleMessage(message, this.telegramService);
  }
}
