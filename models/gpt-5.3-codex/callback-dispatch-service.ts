...
export class CallbackDispatchService {
  ...
  private async handleCallbackQuery(callbackQuery: TelegramCallbackQuery): Promise<void> {
    const context = this.buildCallbackContext(callbackQuery);
  ...
  private buildCallbackContext(callbackQuery: TelegramCallbackQuery): CallbackContext | null {
  ...
  private async handleRewriteAction(context: CallbackContext): Promise<void> {
    ...
    await this.handleRewrite(postId, context.callbackId, context.chatId);
  }
  ...
  private async handleRewriteNow(context: CallbackContext): Promise<void> {
    ...
    await this.answerRewriteInProgress(context.callbackId);
    await this.doRewrite(postId, context.chatId, undefined);
  }
  ...
  private async handlePublishedAction(context: CallbackContext): Promise<void> {
    ...
    await this.handlePublished(postId, context.callbackId, context.chatId, context.messageId);
  }
  ...
  private async handleRewritePublishNow(context: CallbackContext): Promise<void> {
    ...
    await this.answerRewriteInProgress(context.callbackId);
    await this.doRewritePublish(postId, context.chatId, undefined);
  }
  ...
  private async handleUploadImageAction(context: CallbackContext): Promise<void> {
    ...
    await this.handleUploadImage(postId, context.callbackId, context.chatId);
  }
  ...
  private async handleSearchImagesAction(context: CallbackContext): Promise<void> {
    ...
    await this.handleSearchImages(postId, context.callbackId, context.chatId);
  }
  ...
  private async handleBackToRewriteAction(context: CallbackContext): Promise<void> {
    ...
    await this.handleBackToRewrite(postId, context.callbackId, context.chatId);
  }
  ...
  private async handleResetFromRewrite(context: CallbackContext): Promise<void> {
    ...
    await this.doRewrite(postId, context.chatId, undefined, false);
  }
  ...
  private async handleSelectImagesAction(context: CallbackContext): Promise<void> {
    ...
    await this.handleSelectImages(postId, context.callbackId, context.chatId);
  }
  ...
  private async answerRewriteInProgress(callbackId: string): Promise<void> {
  ...
  private async fetchApprovedPost(postId: number, chatId: string): Promise<PostWithDetails | null> {
    try {
      ...
      logger.error(
        { postId, chatId, error: e instanceof Error ? e.message : String(e) },
  ...
  private async finalizeReviewMessage(
    ...
    try {
      ...
      logger.error(
        { chatId, messageId, error: e instanceof Error ? e.message : String(e) },
  ...
  private async sendNextForReview(chatId: string): Promise<void> {
    try {
      ...
      logger.error(
        { chatId, error: e instanceof Error ? e.message : String(e) },
  ...
  private buildReviewPostText(post: PostWithDetails | PostForReview): string {
    ...
    const channelSlug = this.buildChannelSlug(channelTelegramId, post.channel_id);
    const origUrl = this.buildSourceUrl(channelSlug, post.telegram_msg_id);

    return `<b>${escapeHtml(post.channel_name || '')}</b>\n\n${escapeHtml(post.summary || '')}${origUrl}`;
  }

  private buildChannelSlug(
    channelTelegramId: string | number | null | undefined,
    channelId: string | number | null | undefined
  ): string {
    return (channelTelegramId || channelId || '').toString().replace('@', '');
  }
  ...
  private buildSourceUrl(
    ...
    if (!channelSlug || !telegramMessageId) {
      return '';
    }

    const sourceUrl = buildTelegramPostUrl(channelSlug, telegramMessageId);
  ...
  private async handleBackToRewrite(
    ...
    const channelSlug = this.buildChannelSlug(post.channel_telegram_id, post.channel_id);
    const origUrl = this.buildSourceUrl(channelSlug, post.telegram_msg_id);
  ...
  private async sendImageSelection(chatId: string, postId: number, imageState: ImageState): Promise<void> {
  ...
  private async sendImageMediaGroup(
    chatId: string,
    postId: number,
    imageState: Pick<ImageState, 'foundImages' | 'originalLocalPath' | 'hasOriginalLocal'>
  ): Promise<void> {
    const downloadable = (
      await Promise.all(
