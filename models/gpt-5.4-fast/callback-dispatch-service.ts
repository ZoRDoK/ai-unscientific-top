...
import { formatPostTextWithoutSource } from '../shared/utils/post-formatters.ts';
import {
...
type ReviewablePost = PostWithDetails | PostForReview;
...
export class CallbackDispatchService {
  ...
  private async handleMessage(message: TelegramMessage): Promise<void> {
    ...
    await this.handlePendingTextOperation(pending, chatId, text);
  }
  ...
  private async handlePendingTextOperation(
  ...
  private async doRewrite(
    ...
    try {
      ...
      const channelId = this.getChannelId(post);
      ...
      const newText = this.buildPostTextWithSource(
        post?.channel_name,
        summary,
        channelId,
        telegramMessageId
  ...
  private buildReviewPostText(post: ReviewablePost): string {
    return this.buildPostTextWithSource(
      post.channel_name,
      post.summary,
      this.getChannelId(post),
      post.telegram_msg_id
    );
  }

  private async buildReviewReplyMarkup(post: ReviewablePost) {
  ...
  private async sendFreshPostForPublish(postId: number, chatId: string): Promise<void> {
  ...
  private async getSelectedImageLocalPath(postId: number): Promise<string | undefined> {
    const freshPost = await this.backend.getPost(postId);
    ...
    return selectedImage?.local_path ?? undefined;
  }

  private async sendMessageOrPhotoPreview(
    chatId: string,
    text: string,
    imagePath: string | undefined
  ): Promise<void> {
    if (imagePath && (await fileExists(imagePath))) {
      await this.telegramService.sendPhoto(chatId, imagePath, {
        caption: text,
        parse_mode: 'HTML',
      });
      return;
  ...
  private async handleSelectImage(
    ...
    try {
      ...
      await this.sendFreshPostForPublish(postId, chatId);
    } catch (_e: unknown) {
  ...
  private async doRewritePublish(
    ...
    try {
      ...
      const selectedPath = await this.getSelectedImageLocalPath(postId);

      await this.sendMessageOrPhotoPreview(chatId, newText, selectedPath);
  ...
  private async doSearchImages(postId: number, chatId: string, query: string): Promise<void> {
    ...
    try {
      ...
      if (!result.success) {
        await this.handleImageSearchFailure(postId, chatId, normalizedQuery, result.error);
        return;
      }
  ...
  private normalizeSearchQuery(query: string): string {
    return query.trim().replace(/\s+/g, ' ').slice(0, 400);
  }

  private async handleImageSearchFailure(
    postId: number,
    chatId: string,
    ...
    if (error === 'no_images_found') {
      await this.telegramService.sendMessage(chatId, `😕 No images found for query: ${normalizedQuery}`);
      return;
    }

    if (error === 'no_valid_images') {
      await this.telegramService.sendMessage(chatId, `😕 Could not find valid images in 6 attempts`);
      return;
    }
  ...
  private async handleBackToRewrite(
    ...
    const text = this.buildPostTextWithSource(
      post.channel_name,
      post.summary,
      this.getChannelId(post),
      post.telegram_msg_id
  ...
  private async getImageState(postId: number): Promise<ImageSelectionState> {
    const post = await this.backend.getPost(postId);
    const images = post?.images || [];
